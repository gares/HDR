...
\begin{elpicode}
goal> of X T. % never returns 
\end{elpicode}
Indeed by applying the rule:
\begin{elpicode}
of (app H A) T :- of H (arr S T), of A S.
\end{elpicode}
one assigns \elpi{app H1 A1} to \elpi{X} and the runs the subgoal
\begin{elpicode}
of H1 (arr S1 T1). 
\end{elpicode}
to which the same rule applied, and that in turns assigns to \elpi{H}
...
popping the first alternative, returning to the most recent choice point.
...
\elpi{X'} instead of thinking of an appropriate name is payd back, with
...
expression must be placed under a \elpi{pi y} (we picked \elpi{y} to strieve it
