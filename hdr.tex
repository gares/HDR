...
\newmintinline[elpiinline]{elpi.py:ElpiLexer}{fontsize=\small,autogobble,escapeinside=~~,mathescape=true,frame=leftline,framerule=0pt,framesep=1em}
...
\newmintinline[rocqinline]{theorem.py:XCoqLexer}{fontsize=\small,autogobble,escapeinside=~~,mathescape=true,frame=leftline,framerule=0pt,framesep=1em}
...
The \elpiinline{leaf} and \elpiinline{node} symbols can be used to
...
\elpiinline{(tree 46 (tree 93 leaf leaf) leaf)}
...
written in Elpi does not need this feature. In~\cite{fissore:hal-05026472} we
...
example the constant \elpiinline{c}: \elpiinline{((x\ lam y\ x) c)}
evaluates to \elpiinline{(lam y\ c)}.
...
assigning to it a type are limited to the subgoal $\mathrm{of}~(F~c)~T$,
...
to global objects. While \elpiinline{constant}, 
\elpiinline{inductive} and \elpiinline{constructor} are opaque
...
\elpiinline{global} term constructor and its variant
\elpiinline{pglobal} dedicated to universe-polymorphic global
object.
...
three binders: \elpiinline{fun} and \elpiinline{prod} for abstraction
and \elpiinline{let} for abbreviations.
Other than the higher order argument, the detail common
to all of the them is the \elpiinline{name} argument. It stands for
a pretty printing information, e.g. the bound variable as written by
the user. It is important to remark that \elpiinline{name} is
...
is expected to either declare a \elpiinline{decl} or a \elpiinline{def}
...
As a result of the convention, one can call \elpiinline{rocq.typecheck}
deep inside a term. As described in~\ref{sec:API} a predicate can
be equipped with a \ocamlinline{ctx_readback} function such
as \ocamlinline{proof_context} in the code below. The \ocamlinline{term}
...
Indeed here the \ocamlinline{t} and \ocamlinline{ety} values
are read back in the \ocamlinline{proof_context.env} Rocq context,
that is also passed to the type checker in order to synthesize the
type of \ocamlinline{t}, and later (in the omitted code) to unify
the inferred type with the expected one \ocamlinline{ety} when provided.
The \ocamlinline{ioargC} lifts a contextual conversion to the option
type: if actual data is provided, then it is converted.

The only missing piece is \ocamlinline{sigma}, the environment assigning
type judgements to holes.

\section{HOAS of holes (missing terms)}

Each Rocq hole as an entry in \ocamlinline{sigma} that looks like a
typing sequent. In Elpi we represent it as a constraint \elpiinline{evar}.
...
Note how \elpiinline{declare-evar} uses the CHR meta level to craft
a goal with a precise context \elpiinline{Ctx}, no matter the context
under which \elpiinline{declare-evar} is generated. Ineed a Rocq API
...
\elpiinline{declare-evar}, and hence an \elpiinline{evar} constraint
in Elpi.

The code above also relates a Rocq hole \elpiinline{Ev} and an
Elpi unification variable \elpiinline{RawEv}. We want the following
invariants to be hardwired on \elpiinline{Ctx => evar RawEv Ty Ev}.

\begin{itemize}
  \item \elpiinline{Ev} can only be assigned to a \elpiinline{R} if
        \elpiinline{R} has type \elpiinline{Ty} in context \elpiinline{Ctx}
  \item \elpiinline{RawEv} can only be assigned to a \elpiinline{X} if
        \elpiinline{X} elaborates to a term \elpiinline{R} such that
...
In the code above \elpiinline{rocq.elaborate-skeleton} is an API
very similar to \elpiinline{rocq.typecheck} but that does not modify
...
that Rocq runs on any input term written by the user, while \elpiinline{rocq.typecheck}
...
A command is declared with \rocqinline{Elpi Command} followed by a name.
...
can be added to a command via the \rocqinline{Elpi Accumulate}.
...
A command \texttt{hello} is executed with \rocqinline{Elpi hello} followed
by arguments. The \elpiinline{argument} data type has constructors
...
The simple program above calls the \elpiinline{rocq.say} builtin whenever the
command is passed a string. Since the entrypoint \elpiinline{main} has
no rule for arguments of the form \elpiinline{int X}.
...
the entry point is called \elpiinline{solve} that receives as
...
In the example above the tactic expects to receive a string \elpiinline{ID};
...
rules. The \rocqinline{Elpi Db} commands declares a data base.
...
Elpi rules can be given a name via the \elpiinline{:name} attribute.
...
Here we craft a new command \rocqinline{set_age} that does use this API.
...
is the simplest evidence of it: \elpiinline{TheNewRule} contains a pice
of Elpi code built from the \elpiinline{age} predicate and the two arguments
passed to \elpiinline{main}. Since construction/inspection are conflated
...
organized into rules, meaning that \elpiinline{age Name Age} is an entire code
unit one can build or inspect. In comparison the OCaml code for the
\ocamlinline{age} function is a single expression (a single unit):
...
The code below respects these points. Remak \ocamlinline{s} binding now
...
a database. The tactic \rocqinline{to_bool} transfers a goal from the realm
of propositions to the one of computable (boolean) tests (decidable propositions).
The command \rocqinline{register_decision} takes a proof of equivalence
...
tests: it is queried by \rocqinline{to_bool} and populated by \rocqinline{register_decision}.

It is customary to organize Rocq-Elpi code in this very way. The key feature
one obtains is that the \rocqinline{to_bool} is extensible: when a new
predicate is declared and a new test defined, it is sufficient to call
\rocqinline{register_decision} to extend \rocqinline{to_bool} with the new
predicate.

We assume a Rocq environment with the following lemmas. The \rocqinline{reflect}
predicate links a proposition with a boolean test: \rocqinline{reflect P b}
means \rocqinline{P} holds if and only if \rocqinline{b = true}.
...
The \rocqinline{to_bool} tactic is built as follows a top of the
database.
...
standard Rocq \rocqinline{refine} tactic) with \rocqinline{elimT lp:P _}.
The hole stands for the proof of \rocqinline{b = true}, i.e. the subgoal
\elpiinline{G1}.

The most interesting part of this example is how the dabase is populated.
We shall write a \elpiinline{compile} procedute that given \rocqinline{evenP}
and \rocqinline{andP} synthesizes the following Elpi code
...
\rocqinline{evenP} and \rocqinline{andP} and remark how each \elpiinline{pi}
...
\rocqinline{reflect lp:Ty _} we know that the second argument \elpiinline{P}
...
the proof obtained by the recursive call, a proof of \rocqinline{reflect lp:S _}
hene an argument we can pass to the second argument as in \rocqinline{lp:P lp:h}
...
The code of the \rocqinline{register_decision} command located the
...
The Rocq snippet calls the \rocqinline{to_bool} tactic in a scenario
...
unary one, i.e. $i=1$, and we write $U$ for any Rocq universe, i.e. \rocqinline{Prop}.
...
\rocqinline{nat} is \rocqinline{is_nat : nat → U} and
the translation of \rocqinline{3 : nat} is a proof that \rocqinline{is_nat 3}.
...
is has the expected type, i.e. \rocqinline{0} with \rocqinline{is_zero};
\rocqinline{1} with \rocqinline{is_succ 0 is_zero}, and so on.

The translation becomes interesting when the type has parameters such as
\rocqinline{A} in \rocqinline{list A}. In that case the translation builds
a predicate parametric in \rocqinline{A} and in its translation. For example
the translation of \rocqinline{list A} is a predicate
\rocqinline{is_list : ∀A, (A → U) → (list A → U)},
and the translation of the constructors \rocqinline{nil} and \rocqinline{cons}
are:
\begin{rocqcode}
...
that is has the expected type. When the term is of type \rocqinline{A},
the type parameter of the container, then it comes with a proof of \rocqinline{isA},
the predicate parameter associated with \rocqinline{A}.
...
The induction principle generated by Rocq is shallow: \rocqinline{P} is
only available on the immediate subterms of type \rocqinline{rtree}, that
...
That principle is weak since no element of \rocqinline{l} in \rocqinline{Node A l}
verifies \rocqinline{P}. In \cite{tassi:hal-01897468} I study the synthesis
for a stronger principle where \rocqinline{P} holds deep inside \rocqinline{l}.
...
translation of \rocqinline{l} and its type in order to systematically craft
the hypothesis \rocqinline{is_list (rtree A) P l} that gives access to
\rocqinline{P} on all elements of \rocqinline{l}.
...
\rocqinline{is_rtree A is_A t} holds. This extra argument is
synthesized automatically.

For non-containers like \rocqinline{nat}
we prove \rocqinline{∀n, is_nat n} by induction on \rocqinline{n} (no different
than how we proved \rocqinline{is_nat 3} before). For containers we prove
...
of predicates:
\begin{itemize}
  \item the one we are definig, e.g. \rocqinline{nat_is_nat} fits
  \item any \rocqinline{P : t -> U} we are defining by induction on the whole
    type \rocqinline{t}, e.g. the \rocqinline{P} of an induction principle
\end{itemize}
...
The proof is a straightforward induction on list \rocqinline{l}.
...
The last term satisfies the premise of \rocqinline{His_Node}, that in turn
gives the user of the induction principle access to the property \rocqinline{P}
on all the elements of \rocqinline{l}.
...
It reads: if \rocqinline{eqA} is a correct test on all elements of \rocqinline{l},
then \rocqinline{list_eq A eqA} is a good test for \rocqinline{l}.

The interesting case the the equality test for rose trees, that reuses
the equality test for lists \rocqinline{list_eq} passing the recursive call
at the \rocqinline{eqA} argument.
...
What is remarkable is that the correctness proof for \rocqinline{rtree_eq}
reuses the correctness proof for \rocqinline{list_eq}.
...
The main code mattern matches over the first \rocqinline{rtree} and 
pre-computes its tag and fields. The it calls a common piece of
code \rocqinline{eqb_body} that will test if the tags agree and proceed
if it is the case.
...
When the tags agree, \rocqinline{eqb_body} extracts the fields
of the second rose tree and calls \rocqinline{eqb_fields}.
Since the two fields have, respectively, type 
\rocqinline{fields_t t1} and \rocqinline{fields_t t2} a cast is necessary.
...
right type, possibly using the recursive call to \rocqinline{rose_eqb}.
...
chained. Both \rocqinline{rose_eqb} and \rocqinline{eqb_fields} have a number
...
OCaml, the type cast that uses the evidence provided by \rocqinline{pos_eq_dec}
...
Algebra tactics is a port (and improvement) of the \rocqinline{ring}, \rocqinline{field} and
\rocqinline{lra} tactics to the Mathematical Components library~\cite{sakaguchi:LIPIcs.ITP.2022.29}.
...
has to compare the abelian group instance \elpiinline{U} with the ring
instance \elpiinline{R} the tactic is applied to, possibly in the context
\elpiinline{C} of a ring morphism.
...
the \elpiinline{rocq.unify-eq} deep inside the goal to identify
...
The \rocqinline{ring} tactic takes about five seconds to solve it.
...
names of modules themselves. It allows to create the illusion that \rocqinline{A.x}
and \rocqinline{A.y} are items from the same module \rocqinline{A} even if
the two \rocqinline{A}s have different (long) names, eg \rocqinline{X1.A.x}
and \rocqinline{X2.A.y}.
...
It worth pointing out how the definition of \rocqinline{more_stuff}
...
We take the liberty to challange the reader to write the \rocqinline{to_bool}
