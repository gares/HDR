...
the constants in \elpi{S1} may occur in \elpi{T1}, and the
...
In~\cite{dunchev15lpar} we  assess the performances of 
Elpi on a set of synthetic benchmarks and
a real application. Synthetic benchmarks are divided into three groups:
first order programs from the Aquarius test suite~\cite{aquarius} (the
crypto-multiplication, $\mu$-puzzle,
generalized eight queens problem and the Einstein's zebra puzzle);
higher order programs falling in \thefragment{} from~\cref{fig:bench}; and an higher
order program falling outside \thefragment{} taken from the test suite of
Teyjus normalizing expressions in the SKI calculus.

The programs in \thefragment{} are respectively type checking lambda terms using
the \verb+of+ program discussed in~\cref{sec:hello}
and reducing expressions like
$5^5$ in the syntax of Church numerals using a call by value and by name
reduction strategy.
The typechecking test was specifically conceived to measure the cost of
moving under binders: the type checked terms are projections, i.e.,
are mainly made of \elpi{lam} nodes.
...
The data in the 
table shows that Elpi shines on programs in \thefragment, and compares well
outside it. The alternating performance Teyjus on the reduction tests has
to be attributed to the explicit substitutions machinery~\cite{NADATHUR200235}
when employed to cross binders:
by its very nature explicit substitutions fit well a lazy reduction strategy like
call by name (even if some space overhead is visible).  On the contrary explicit substitutions
are counterproductive in the call by value case since the program,
by fully traversing the redex argument,
...
If one makes Teyjus artificially
push explicit substitutions in the call by name case too,
it halves memory consumption but degrades the performances by 10 seconds,
confirming the time we see in the call by value case is dominated by the overhead of
...
expects call by value is faster than call by name in computing the normal form of $5^5$
since it avoids duplicating non-normal terms.

In~\cite{dunchev15lpar} we also compare Elpi and Teyjus
on Helena, a type checker for the formal system
...
Our Elpi implementation follows it closely, and
naturally falls in \thefragment{}.
Nevertheless, the Elpi code is much simpler than the corresponding OCaml code
and consists of just 50 rules.

The Grundlagen is a theory comprising definitions and proofs
for a total of 6911 items (circa 8MB of data).
When we run the benchmarks, around 2015, Teyjus seemed to have
a fixed maximum heap size of 256MB that in turn
limited it to the verification of the first 2615 items.
In the table we compare pre-processing (Pre) time like parsing, compilation
or elaboration, and verification (Ver).
We compare Elpi with Helena, Teyjus, and Rocq.
The Rocq system implements a
...
the ones of a state-of-the-art interactive prover.
...
This data suggests that Elpi performs quite similarly to Teyjus on first order
programs. The difference in performance, in favour of Elpi, is probably due to
the more efficient garbage collector Elpi uses (the OCaml one). The same
happens on programs manipulating higher order data outside~\thefragment{}.

Inside~\thefragment{} Elpi is sensibly more effecient.
Teyjus matches Elpi's performance only if
the program benefits from a lazy handling of substitutions, and even there
the memory overhead is significant.

Given that Elpi targets Rocq as its host application, the most interesting
benchmark is how it compares, on a similar task, with its OCaml implementation.
Elpi is about 10 times slower than Rocq. Considering Elpi is an interpreter
this difference is somewhat expected and we find it acceptable. After all,
when Rocq provides an efficient routine an Elpi program can take advantage
of it by simply calling it. What Elpi is expected to provide is a more
high level language to maniplate syntax trees, fast \emph{enough} to be useful
in practice.
...
higher-order argument \elpi{F} is. 

Since the syntax for \elpi{func} requires
...
The history of logical frameworks and interactive provers is intertwined.
Maybe Isabelle is most well known prover rooted on that idea: it hosts
various foundational languages, from the set theory Isabelle/ZF to first order
classical logic Isabelle/FOL and the most widely known higher order logic
Isabelle/HOL. The main
drawback in its design, to me, is that the logical framework is so weak,
from a proof theoretic point of view, that one cannot really talk about
these formal systems in it, e.g., one cannot relate a result in Isabelle/ZF 
with one in Isabelle/HOL, nor reuse the automatic proof search tools
that make Isabelle/HOL so practical when working in Isabelle/ZF. In
support of this observation we mention that Paulson implemented
Isabelle/ZF and later mechanised ZF(C) in Isabelle/HOL~\cite{zfhol}.

In a way each Isabelle/X is a different,
incompatible, prover with Isabelle/Y. The economy made by sharing a common
implementation infrastructure seems irrelevant compared to the economy made
by sharing mechanized theorems. One can try to solve this problem by
reconsiliating different theories via an external translation, as the
ones provided by Dedukti~\cite{dedukti}, but in that way he likely loses
the benefits of sharing an implementation infrastructure.

What I see as an emerging third approach is to use Rocq's logic as the
``logical framework''. The calculus of inductive constructions is not designed
for that, for example the function space is not the right one for an
HOAS encoding of binders, and it is also unrestricted with respect to
duplication. Still Gallina features a programming language that can
be used to encode other formal systems, and proof theoretically speaking it is
strong enough to contain them, reason about them and share, at some
extent, theorems.
Maybe the most shiny example of this approach is Iris. Iris is a separation logic
that is proved sound in Rocq, that thanks to its proof mode it
can be used to reason about imperative programs. When possible
pointers and heap locations are related to mathematical objects, like
lists made of cons and nil, and imperative algorithms to their
functional specification, the same specification other...
