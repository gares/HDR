...
In life, there are many questions without an answer. One of them is, apparently,
what this HDR document is for, other than fulfilling an administrative duty.
The University of Nice suggests wrapping with skinny introduction and conclusion
the copy-pasting of abstracts from papers published after the PhD.
While that would have been easier to write and shorter, it would also have been
a completely useless exercise, resulting in a document that, in ten years from
now, I myself would have no pleasure in skimming through. So I took the
opportunity to write a document that brings together most of the research I did
in the last ten years around a single thread -- the Elpi language -- and puts it in
perspective, trying to distill some conclusions, such as what worked and what
did not. It will not cast a shadow on earth, but hopefully I will not be ashamed
to put it online either.

So here it is how it all begins.

\subsection{Beyond the Odd Order Theorem}


I was fortunate to be part of a major mechanization project that culminated in
the formalization of a 250-page proof in Rocq~\cite{DBLP:conf/itp/GonthierAABCGRMOBPRSTT13}.
After its completion, the natural question arose: what made such an achievement
possible, aside from the leadership of Georges Gonthier? More importantly, how
could others be empowered to construct similarly impressive machine-checked
proofs?

In my humble opinion three main factors contributed to the project's success: (1) sound engineering
practices -- nothing new, but too often overlooked by the formal-proofs community; (2) the
formalization technique known as boolean reflection (or small-scale reflection),
which carves out a lightweight ``classical mathematics'' framework in Rocq
i.e. making excluded middle, function extensionality and proof irrelevant available
when constructivism allows; and (3) the ``creative'' programming of the
Rocq elaborator.

Our focus was on the third point. These programs enabled Rocq to behave like an
informed reader -- one who knows some basic facts and can combine them using
well-known rules. At the time, Rocq was often seen as a ``dumb reader'' who
required every detail to be spelled out. These new programs changed the game for
the Odd Order Theorem, making it possible to use mathematical notations that
convey a lot of information implicitly, by convention.

Our goal hence became to help users craft such programs, since programming the elaborator as we
did was challenging. Structuring the library so that knowledge could be
organized by the author and retrieved by these programs was even harder. The
programming mechanism, called Canonical Structure inference~\cite{tassi13}, is closely related
to the language of Unification Hints~\cite{asperti09} I studied during
my Ph.D. Around the same time, Rocq was extended with type classes~\cite{fctc}, which were
used for similar purposes and immediately raised similar programming issues.

To me, these programs were strikingly similar to Prolog. Our code was organized
...
that family, and thanks to Assia Mahboubi and Stephane Legrand, I was invited to
give a talk at a workshop they were hosting.
...
that his $\lambda$Prolog language~\cite{10.1093/logcom/1.4.497} was probably what I was looking for. Since I
knew nothing about it, I could only nod and ask to continue the discussion
offline.

Dale was kind enough to set up a meeting with Claudio Sacerdoti and myself
to discuss about a language well suited to describe the elaboration process
of an interactive theorem prover.

\subsection{A snowy day}

On that day ``Paris'' was frozen, in the sense that a supposedly
apocalictic show storm convinced authorities to close buildings and other facilities,
reduce public transportations to the bare minimum. For some reasons
only the PCRI building was open, so the 3 of us agreed to buy a sandwich,
walk up the hill (with boots) and meet in a desert building.

After two hours of gentle introduction to $\lambda$Prolog, after having
savoured the elegance of the \elpi{copy} predicate, I clearly remember
I said ``sold...'', but I also completed the sentence with
``How do I do evars?''.
...
notably contains binders. But terms also contain holes, evars in
Rocq slang, that are the other beast one has to tame in order
to implement an elaborator and, in general, the outern layers of a tool
like Rocq. The answer was not on the shelf, so that was likely
the beginning out this line of research.

$\lambda$Prolog was not an idealized programming language, it had
a full implementation in the Teyjus~\cite{teyjus} system.
In a visit to Bologna, together with Claudio Sacerdoti, we
could write a toy elaborator for a dependent type theory in one day,
but we could not find a good solution for evars. Our intuition was
to try to reuse the unification variables of $\lambda$Prolog to
represent evars, but that turned out to be extremely hard since
$\lambda$Prolog do not really give you a way to control their
assignment: and an elaborator takes in input a term with holes
and returns a term where only some holes are assigned, not all of them.
One obvious option was to reify them as it is done in the code of Rocq, i.e.
represent them with numbers and explicitly thread around a state
associating a type and an assignment to each evar. By doing so
the conciseness and elegance of the code we were writing was gone.

I was convinced that it was worth trying devising a variant of
$\lambda$Prolog to overcome this problem, but changing Teyjus or
trying to integrate it in Rocq seemed too hard. Teyjus is
a compiler, generating bytecode for a virtual machine
written in C: adding constructs to the source language would have
required mastering the entire pipeline, not to mention to complexity of
using C code from OCaml, the language in which Rocq is written.
So I wrote a proof of concept interpreter of $\lambda$Prolog in OCaml,
for both better understanding the language, and to ease its integration
with Rocq, or its lightwight twin Matita~\cite{DBLP:conf/cade/AspertiRCT11}.
On that toy interpreter it was easy make experiments, evaluate hacks,
and so on. We did manage to write an elaborator for the
Calculus of Inductive constructions with it, and have Matita run it
on the statements of the theorems in its arithmetic library.
It was working! Almost, at least. The performance were bad, very bad,
like 22K times bad. Still, the experiments we could run on the
prototype made it clear that most of the hacks could be explained in terms of
constraints (as is constraint-logic programming) and rules to manipulate
these constraints.

Together with Claudio Sacerdoti we rewrote the interpreter to make it
reasonably fast and later extend the language with constraints. I gave
this piece of software the name ELPI, that stands for Embeddable
Lambda
Prolog Interpreter.~\cite{dunchev15lpar}
When the language started to differ from $\lambda$Prolog,
i.e. when we mixed it with Constraint Handling Rules~\cite{FRUHWIRTH199895},
we started to use Elpi for the language itself, and not just the implementation.

Since the very beginning Elpi was conceived to be embedded into a host
application such as Rocq. It never aimed at being a general purpose
programming language or to scale to large applications. It always had
a niche domain of application (compared to mainstream programming languages).
We believe this clear focus was instrumental to mage good use of the resources
we had.

\section{Elpi}

Elpi is free software, it comes as a library distributed under the terms
of the LGPL license. It is developed in the open, at
\url{https://github.com/LPCIC} where we gather the interepter \texttt{elpi},
its integration in Rocq \texttt{Rocq-elpi}, and the extensions for
Visual Studio Code \texttt{elpi-lang} and \texttt{roc-elpi-lang}.

This document describes the research we did in the last decade
around this language and using this language.

The first two chapters of this document describe Elpi from A to Z, both as a
language and as an implementation of that language. We credit Ridoux~\cite{ridoux1998lambda}
for the titles of these two chapters and we thank him for writing and HDR
manuscript that we enjoyed reading.
The third chapter describes the integration of Elpi in Rocq
and the fourth one surveys the application developed using Elpi,
most of which are Rocq extensions.

Sections \ref{sec:milner} and \ref{sec:tb} contain full examples of Elpi
and Rocq-Elpi programs tested with Elpi version 3.1 and Rocq-Elpi version 3.0.
...
Again, we favor a type assignment over a simple arity. \elpi{prop} is the type
of executable code, meaning that \elpi{size}, when applied to two arguments,
...
built-in \elpi{is} operator, which evaluates its right-hand side and unifies
...
Note that \elpi{(x\ lam y\ x)} is a function of \elpi{x} (a variable of the
...
\elpi{uvar as E} binds the unification variable \elpi{E} to a hole in the
...
assigned, the goal should be resumed (i.e. \elpi{E} is the trigger for
...
\elpi{of (app D D) T}, since the goal \elpi{S = arr S T} fails (because of
the occurs check).
...
variables are \emph{frozen}: a unification variable \elpi{F} applied to
\elpi{x} and \elpi{y} is presented to constraint handling rules as
\elpi{uvar f [x,y]} for a fresh constant \elpi{f}. This lightweight form of
...
the constants in \elpi{S1} may occur in \elpi{T1}, and possibly different
ones in \elpi{S2} may occur in \elpi{T2}: one needs to align \elpi{T1}
with \elpi{T2} before equating them -- more precisely, to move \elpi{T2} into
the space of names \elpi{E1}.
...
active goal. Finally, action (4) operates on the entire set of suspended
...
\elpi{X} is instantiated, concluding by producing an incomplete tree (which the
...
The rules acting on the store $\mathcal{R = \vecL{\mathbb{C}}}$ are
...
The rules in Figures~\cref{fig:basic-interp-c} and~\cref{fig:basic-interp-l}
are applied in the specified order. Rules~\ref{rule:delay} and~\ref{rule:resume}
...
popping the last alternative, returning to the most recent choice point.
...
cut-to alternative, and that the value passed to $\bchain$ by rule
...
The starting point is the so-called ``active constraint,'' which, in the context
...
This description of how CHR rules update the constraint store is essentially the
standard operational semantics~\cite{10.1007/978-3-540-27775-0_7}, adapted to
constraints composed of a program $\prog$ and an applicative term $c$. As
...
filter to discard rules for predicates not relevant to the constraint
...
context made of \elpi{r} and \elpi{s} is:
...
trigger $t$ of the active constraint. According to our experience, this is
usually what the user needs and drastically reduces the number of permutations
to consider. For reference, the syntax to opt out is as follows:
\begin{elpicode}
...
\elpi{X'} instead of thinking of an appropriate name is repaid, with
...
expression must be placed under a \elpi{pi y}. Similarly, the temporary
...
the implication operator. For example if the \elpi{lam}
...
\begin{elpicode}
  code Ty Result :- 
    % we call of under the binder for x
...
here we call an predicate \elpi{api} under the object language binder
for \elpi{x}.
...
If \elpi{this} and \elpi{that {rev D}} are predicates, the latter expands to
...
\elpi{glo} term constructor to enable the setup of a minimal environment
for our examples.
...
\noindent
The only detail worth describing is the use of the \elpi{prune} builtin
...
The \elpi{eqbar} predicate traverses concrete type expressions and suspends
...
\elpi{prop} type gathers all predicates, not just \elpi{of}. We do not need
to add a rule to skip any unrelated predicate the program may have loaded with
implication, because the \elpi{constraint of ?- gammabar eqbar theta
...
quantifications. The key is the \elpi{copy} predicate, which implements
...
$$
In this case, the first use of \elpi{f} is on an empty list, which leaves the
...
This
becomes clear when one has a program \elpi{A, B => D} and wants to sneak in a
...
While \elpi{pi x\ F = x} logically fails because \elpi{x} is out of scope, its
...
\elpi{N < M}. Therefore, it is difficult to imagine a sensible warning for
this issue.
...
\ocaml{AppArg} node. Heap terms are longer lived, typically kept alive by a
...
data type. Below, we present the declaration for the \ocaml{option} type.
...
\ocaml{Pred} constructor, as shown in~\cref{fig:ffi}. The former is a GADT that
  ...
  let getenv : BuiltInPredicate.t =
    Pred("getenv",
      ...
      Easy ("Like Sys.getenv"))),
       (fun s _ ~depth ->
          try !:(Some (Sys.getenv s))   % !:x  is a notation for  (), Some x
          with Not_found -> !: None))   % the GADT recursion adds a final  ()
\end{ocamlcode}
