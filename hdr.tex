...
TODO: introduce HOAS/ltree syntax
...
of rules, possibly written by different authors. It is not impossible for two
files, e.g., \texttt{file1} and \texttt{file2}, to contain declarations for a
predicate with the same name, say \texttt{p}.

When the types for \texttt{p} do not match, Elpi fails immediately and warns
the programmer. But when the types match, it may happen that the predicate
changes meaning or computational complexity, especially in the case of failure.

A simple solution could be to include the file name as part of predicate
names, e.g., \texttt{file1.p} and \texttt{file2.p}.

To alleviate the syntactic overhead, Elpi provides two syntactic facilities:
\texttt{namespace} and \texttt{shorten}. The former prefixes all predicate
names contained in a block, while the latter makes predicates accessible via a
short name inside a file or block.

In the following snippet, the predicate \elpi{p} lives in the namespace
\elpi{n} and is thus accessible via the long name \elpi{n.p} (outside of the
namespace).
...
Spilling can be nested. Its elaboration is akin to the generation of the
...
\noindent.
This is frequent when spilling combined with quotations, as we
will better describe in~\cref{sec:quotations}. For example
here we call an \elpi{api} under the object language binder for
\elpi{x}.
...
spilled expression must be executed under the implication.

\paragraph{Spilling ambiguities}

Not all that glitters is gold. Sometimes the closest execution point may not
be what the programmer intends.
...
If \elpi{this} and \elpi{that} are predicates, the latter becomes
\elpi{(rev D TMP, that TMP)}, meaning that \elpi{D} is reversed when
\elpi{TodoForLater} is executed, and not when the todo list is generated, as
one might have intended.

According to our experience, the closest execution point is more often than
...
\elpi{global} term constructor to enable the setup of a minimal environment
for our examples.
