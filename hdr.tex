...
leaves any choice points behind. The signature reported below uses the arrow
to separate inputs from outputs and means that \elpi{once} behaves as a
...
precondition is not met, we say that \elpi{map} is miscalled but still runs fine
even if it may leave choice points. Miscalled functions are tracked by
the static analyzer and considered as relation for the analsysis of the
surrounding code. In other words the signare of \elpi{map} is allowed to
degrade to the following one, that requires nothing on its input but also
guarantees nothing on the choice points it may leave.
\begin{elpicode}
...
ongoing computation in order to reason about the scope of a cut.
While this facilitated proofs by making invariants easier
...
than the list of alternatives used in~\cref{sec:sem}. Proving the two
semantics equivalent is ongoing work.

From a practical perspective it is unclear to me if programs written in
Rocq-Elpi would truly benefit from a program logic. Rocq features five extension
languages to our knowledge (Ltac1, Ltac2, MTac, Meta-Rocq and Elpi) and only
Meta-Rocq has one: it uses Gallina itself as a programming language, so, at
least in principle one could use Rocq to prove properties of meta programs in
Rocq itself. While I find it very intriguing intellectually, this possibility
found, at the time of writing, almost no application. So I'm skeptical
providing a program logic for the full Elpi language would make it significantly
more practical. Hence it will not be a priority of mine, but I would love to read
such a program logic written on paper by someone else.

\subsection{Elpi as a foundational language}

I have always seen, and still see, Elpi as a programming language
that puts \emph{practical} before \emph{foundational}, i.e., disregarding
the possibility of looking at Elpi programs, or their execution, as a formal
...
Even if we did not choose to follow the footsteps of 
Twelf~\cite{twelf},
Beluga~\cite{DBLP:conf/cade/PientkaC15,10.1007/978-3-642-12251-4_1} or
MMT~\cite{RABE20131} when it comes to being foundational,
the literature that describes these systems has been
...
Isabelle/ZF and later mechanised ZFC in Isabelle/HOL~\cite{zfhol}.

In a way each Isabelle/X is a different,
incompatible, prover with Isabelle/Y. The economy made by sharing a common
implementation infrastructure seems little compared to the economy one could
make by sharing mechanized theorems. One can try to solve this problem by
reconciliating different theories via an external translation, as the
...
What I see as an emerging approach is to use Rocq's logic as the
...
HOAS encoding of terms with binders, and it is also unrestricted with respect to
duplication. Still Gallina features a programming language that can
be used to encode other formal systems, and proof theoretically speaking
CIC it is strong enough to contain them, reason about them and share, at some
extent, theorems.
Maybe the most shiny example of this approach is
Iris~\cite{10.1007/978-3-662-54434-1_26}. Iris is a separation logic
that is proved sound in Rocq, and that thanks to its proof mode~\cite{IPM} it
can be used to reason about imperative programs within Rocq.
Moreover, whenever possible, pointers and heap locations are related to
mathematical (immutable) objects like lists made of cons and nil,
via inductive predicates. Then imperative algorithms are related to their
functional specification, and at that level theorems can be shared.
Other examples of frameworks are VST~\cite{VST} implementing a program
logic for C, of the FOL~\cite{FOL} library implementing first order logic,
giving results on its metatheory and letting one write proofs in it.
Even the Mathematical Component library is built this way even if
its framework  is thin, compared to, say, Iris. In particular 
the extensive use of booleans to encode dicidable propositions make
reasoning by excluded middle more practical and it paves the way to proof
irrelevance effectively building a comfortable shell for ``classical reasoning''
(without adding any classical axiom).

If this trend continues, I see two roles Elpi could play.
One is to continue to serve as an extension language, after all these
frameworks needs ecoding and Elpi can help to hide them, see for
example~\cite{TODOIRIS}. Maybe a more intriguing possibility would
be to make a step back an try to retrofit a logical framework in type
theory, in the spirit of~\cite{concon}. If that happens, the techniques
employed to implement Elpi's runtime could
provide a starting point for the logical framework infrastructure
the new type theory requires.
