...
Milner and its collaborators for the ML language. The flavor we 
study here covers equality type variables, the \texttt{''a} abstraction
(in standard ML syntax) for decidable types, i.e. types with an equality test.
...
are depicted below. 
For the sake of this presentation we fix a few builtin type contructors
instead of supporting a generic n-ary type former.
...
of (global "nil")    (all any x\ mono (list x)).
of (global "cons")   (all any x\ mono (x --> list x --> list x)).
of (global "pr")     (all any x\ all any y\ mono (x --> y --> (pair x y))).
of (global "fst")    (all any x\ all any y\ mono (pair x y --> x)).
...
\displaystyle\frac{\tau' = \left\{\bar\alpha_i \mapsto \tau_i\right\} \tau \quad \beta_i \not\in \textrm{free}(\forall \alpha_1...\forall\alpha_n . \tau) \quad \overline{eq}(\tau')}{\forall \alpha_1...\forall\alpha_n . \tau \sqsubseteq \forall \beta_1...\forall\beta_m . \tau'}
$$
...
\begin{array}{ll}
  ...
  \overline{eq}(\mbox{list}~\tau) & ~\mbox{if}~ \overline{eq}_\Theta(\tau) \\
  \overline{eq}(\mbox{pair}~\tau_1~\tau_2) & ~\mbox{if}~ \overline{eq}_\Theta(\tau_1) ~\mbox{and}~ \overline{eq}_\Theta(\tau_2)\\
...
free (uvar as X) L L1 :- if (mem L X) (L1 = L) (L1 = [X|L]).
