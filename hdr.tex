...
\begin{abstract}
This document presents the Elpi, a rule-based extension language
designed for embedding in applications such as interactive theorem provers
(notably Rocq). Elpi is a blend of
λProlog (a higher-order logic programming language) and Constraint Handling
Rules (CHR), enabling expressive manipulation of syntax trees with binders and
holes.

The manuscript describes Elpi as a programming language
contrasting it with Prolog and λProlog. It then describes
its implementation, highligting the key to its efficiency and
ease of integration in host applications. It continues detailing
the integration of Elpi with Rocq and concludes with
a survey that catalogs applications built on
Elpi, identifying the language features that are important to each of them.

Two case studies illustrate Elpi's power. The first one a version of
Hindley-Milner type-inference algorithm with equality types. The second one
is a proof-transfer tool.
\end{abstract}

%% Résumé en français : pas plus de 2000 caractères
\begin{resume} TODO \end{resume}
