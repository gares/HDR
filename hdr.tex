...
In Elpi 3.0 we integrated a determinacy checker~\cite{elpidet} and a new
keyword \elpi{func} to be used in place of \elpi{pred} in the signature
of predicates. The static checker guarantees, essentially, that
...
An indeed \elpi{once} can be flagged as \elpi{func} since it never leaves
any choice points behind it.

An interesting example comes from the portfolio of functioanl programming.
\begin{elpicode}
func map (func A -> B), list A -> list B.
map F [] [].
map F [X|XS] [Y|YS] :- F X Y.
\end{elpicode}
\noindent
...
it paves the way to support, for functioanl predicates body, closer to
functional languages, i.e. where expressions and commands are confused together.
