...
Before delving into the implementation we recall the main idea of this type
inference algorithm that is capable of assigning to let-bound functions
a polymoprhic type, under some conditions. More precisely
the type of a let bound function is universally quantified over all the type
variables that are used \emph{only} to type the function body.
If a type variable ``leaks'' from the body, then it cannot be universally
quantified.

For example the first \ocaml{f} is polymorphic, while \ocaml{h} is not:
\begin{ocamlcode}
...
let b y = (* b : $\alpha \to \mathrm{bool} \times \mathrm{bool} $ *)
  let h x = (* h : $\alpha \to \mathrm{bool}$ *)
    x = y
  in
    (h "foo", h 1)  (* error: $\alpha = \mathrm{string} \not= \mathrm{int} = \alpha$ *)
\end{ocamlcode}
The typing algorithm proceeds pretty much like
the type checker for simply typed $\lambda$-calculus of~\cref{sec:hello}, i.e.,
using existentially quantified type variables to represent
unknown types, and using unification to establish type equality.
The clever step is recognizing that an existentially quantified variable
used to type a function body occurs nowhere else, indicating that the code
makes no assumption on that type. In this case the function
is given a polymorphic type.

In the example above the body of \ocaml{h} compares \ocaml{x} with \ocaml{y} forcing
their type to be the same $\alpha$ that hence escapes the body of \ocaml{h}.
Indeed one is not able to use \ocaml{h}
on arguments of types other than the type of \ocaml{y}. If the
definition \ocaml{b} was allowed (by considering \ocaml{h} polymorphic),
then by running \ocaml{b} on, say, \ocaml{2},
one would get a runtime type error when comparing \ocaml{2} with \ocaml{"foo"}.
