...
Before delving into the implementation, let us recall the main idea behind
this type inference algorithm, which can assign a polymorphic type to
let-bound functions under certain conditions. More precisely, the type of a
let-bound function is universally quantified over all type variables that are
used \emph{only} within the function body. If a type variable ``leaks'' from
the body, it cannot be universally quantified.
...
In this case, the function is given a polymorphic type.
...
than the type of \ocaml{y} by being also used to type \ocaml{x}.
