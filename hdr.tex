...
the abstracts of papers published after the PhD. While that approach would have
...
in the formalization of a 250-page proof in Rocq~\cite{DBLP:conf/itp/GonthierAABCGRMOBPRSTT13}.
...
irrelevance available when constructivism allows; and (3) the creative
...
does not get tired but requires every detail to be spelled out. These new programs changed the game for
...
Our goal became to help users craft such programs, since programming the
elaborator as we did was challenging. Structuring the library so that knowledge
could be organized by the author and retrieved by these programs was even
harder. The programming mechanism, called Canonical Structure
...
On that day, ``Paris'' was frozen -- a supposedly apocalyptic snowstorm convinced
...
assigned -- not all of them. One obvious option was to reify them as is done in
...
overcome this problem, but changing Teyjus or integrating it into Rocq seemed
...
to experiment, evaluate hacks, and so on. We managed to write an elaborator for
...
$\lambda$Prolog—specifically, i.e., when we mixed it with Constraint Handling
...
programming language and to scale to large applications. It always had a niche
domain of application (compared to mainstream programming languages). We
believe this clear focus was instrumental in making good use of the resources
we had.
...
implementation. We credit Ridoux~\cite{ridoux1998lambda} for the titles of
...
Rocq-Elpi programs, tested with Elpi version 3.1 and Rocq-Elpi version 3.0.
