...
and that the user has access to the scope of frozen unification variables.
...
\newcommand{\rulePiM}[1]{
  \begin{myRule}{#1}
    % PI
    \AxiomC{\ensuremath{\runCmdR{\ConsHdNP{\goalCmd{(x + \prog)}{g}{\alts}}\ConsTlNP{\vecL{g}}}{\alts[']}{\subst}}}
    \RightLabelM{\rulePi}
...
carrier the same cut-to alternative and that the value passed to
...
The starting point is the so called ``active constraint'' $A$ that, in the context of Elpi, is
the constraint just declared as $\cst\ A\ t$ by the rule~\ref{rule:delay}.

\begin{enumerate}
\item for each rule $P_1 \ldots P_x\ \backslash\ P_{x+1} \ldots P_n\ |\ C \Leftrightarrow G$
...
\item match all $C_i$ with $P_i$ and run $C$. If it succeeds:
  \begin{itemize}
    ...
    \item return $G$ as a new goal
  \end{itemize}
\item move to the next rule
\end{enumerate}

TODO: explain freezing
TODO: explain sequents, filtering, only hyp in ctx, ctx sorted
TODO: explain trigger overlap
