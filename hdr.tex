...
management but dropping the rule-based nature, and thus automatic context
...
the codebase had two code paths (and thus two bugs) that could sometimes be
...
complete the mechanization of the runtime.
...
One of the reasons is surely that I felt in the ``marmitte'' of the Calculus of
...
the foundational language has always been Rocq to me, leaving Elpi the role of
being practical.
...
Maybe Isabelle is the most well-known prover rooted in that idea: it hosts
...
Isabelle/HOL. The main drawback in its design, to me, is that the logical
framework is so weak, from a proof-theoretic point of view, that one cannot
really talk about these formal systems in it, e.g., one cannot relate a result
in Isabelle/ZF with one in Isabelle/HOL, nor reuse in Isabelle/ZF
the automatic proof search
tools that make Isabelle/HOL so practical. In
support of this observation, we mention that Paulson implemented Isabelle/ZF
and later mechanized ZF in Isabelle/HOL~\cite{zfhol}.\\
In a way, each Isabelle/X is a different, incompatible prover with
Isabelle/Y. The economy made by sharing a common implementation infrastructure
seems little compared to the economy one could make by sharing mechanized
theorems. One can try to solve this problem by reconciling different theories
via an external translation, as the ones provided by Dedukti~\cite{dedukti},
but in that way, one can hardly reason on the translation itself.

What I see as an emerging approach is to use Rocq's logic as the logical
framework. The Calculus of Inductive Constructions is not designed for that;
for example, the function space is not the right one for an HOAS encoding of
terms with binders, and it is also unrestricted with respect to duplication.
Still, Gallina features a programming language that can be used to encode
other formal systems, and proof-theoretically speaking, CIC is strong enough
to contain them, reason about them, and share, to some extent, theorems. Maybe
the most shining example of this approach is Iris~\cite{10.1007/978-3-662-54434-1_26}.
Iris is a separation logic that is proved sound in Rocq, and thanks to its
proof mode~\cite{IPM}, it can be used to reason about imperative programs
within Rocq. Moreover, whenever possible, pointers and heap locations are
related to mathematical (immutable) objects like lists made of cons and nil,
via inductive predicates. Then imperative algorithms are related to their
functional specification, and at that level, theorems can be shared and
tranferred, in a proved-correct way,
with tools like Trocq~\cite{10.1007/978-3-031-57262-3_10}.
Other
examples of frameworks are VST~\cite{VST} implementing a program logic for C,
or the FOL~\cite{FOL} library implementing first-order logic, giving results
on its metatheory and letting one write proofs in it. Even the Mathematical
Component library~\cite{assia_mahboubi_2022_7118596} is built this way,
even if its framework is thin compared to, say, Iris. In practice, the
extensive use of booleans to encode decidable
propositions makes reasoning by excluded middle look native, and it paves
the way to proof irrelevance, effectively building a comfortable shell for
classical reasoning within CIC without adding axioms and hence providing
a base of theorems reusable in other frameworks. For
example~\cite{10461453} reuses some linear algebra from
Mathematical Components to prove correct a C function that multiplies sparse
matrices over IEEE-754 conforming floating point numbers.

If this trend continues, I see two roles Elpi could play. One is to continue to
serve as an extension language; after all, these frameworks need encoding, and
Elpi can help to implement them, or hide them to the user.
See for example~\cite{krebbers25} that gives the user the illusion of
having inductive predicates in the Iris logic.\\
Maybe a more
intriguing possibility would be to take a step back and try to retrofit a
logical framework in CIC, in the spirit of~\cite{concon}. If that
happens, the techniques employed to implement Elpi's runtime could provide a
solid starting point for the logical framework infrastructure the new type
