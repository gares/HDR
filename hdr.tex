...
\newcommand{\CALLHD}[1][]{\ensuremath{\mathbb{C}#1}\xspace}
\def\CALL{\CALLHD}
...
In the syntax of atoms \ATOM we single out the \cut operator (\elpi{!} in the concrete syntax) and the
constraint-declaration \cst one (\elpi{declare_constraint} in the concrete syntax)
since they have dedicated treatment in the operational semantics.
...
\begin{figure}
  ...
  \begin{array}{rlr}
    ...
    \ATOM& ::= \cut \mid \cst\ (\CALLHD\ \vecL{\TERM})\ \vecL{\VAR} \mid \CALLHD\ \vecL{\TERM} \mid \piCmd{\VAR}{\ATOM} \mid \implCmd{{\CLAUSE}}{\ATOM} & \mathrm{atom} \\
    \TERM & ::= \CALLHD\ \vecL{\TERM} \mid \lambda \VAR \bs\ \TERM & \mathrm{term}\\
    \CLAUSE & ::= \clauseCmd{\CALLHD}{\vecL{\TERM}}{\vecL{\ATOM}} & \mathrm{rule}
  \end{array}
  $$
...
\newcommand{\runCmdR}[3]{\ensuremath{\run\ (#3,\ #1)\ #2 \arr r}}
...
\newcommand{\runQuery}[2]{\ensuremath{\run\ (\EmptySubst,\ #1)\ #2 \arr \_}}
\newcommand{\runCmdF}[3]{\ensuremath{\run\ (#3,\ #1)\ #2 \arr \bot}}
\newcommand{\runCmdFQ}[2]{\ensuremath{\run\ #1\ #2 \arr \bot}}
...
\newcommand{\ruleCst}{\ruleName{rule:cst}{\bigtriangleup}}
\newcommand{\ruleTcs}{\ruleName{rule:cst}{\bigtriangledown}}
...
\newcommand{\ruleCstM}[1]{
  \begin{myRule}{#1}
    % suspend
    \AxiomC{\ensuremath{\mathcal{CHR}\ m\ (\prog,c)_v\ \alts['] = (\vecL{g'}, m')}}
    \AxiomC{\runCmdR{\vecL{g'} \atsign \vecL{g}}{\alts}{(s,m')}}
    \RightLabelM{\ruleCst}
    \BinaryInfC{\runCmdR{\ConsHdNP{\goalCmd{\prog}{\cst\ c\ v}{\alts[']}}\ConsTlNP{\vecL{g}}}{a}{(s,m)}}
  \end{myRule}
}
\newcommand{\ruleTcsM}[1]{
  \begin{myRule}{#1}
    % resume
    \AxiomC{\ensuremath{c_v \in m \quad \exists v_i \in v, s\ v_i \not= v_i}}
    \AxiomC{\runCmdR{\ConsHdNP{c} \atsign \vecL{g}}{\alts}{(s,m - (\prog,c)_v)}}
    \RightLabelM{\ruleTcs}
    \BinaryInfC{\runCmdR{\vecL{g}}{a}{(s,m)}}
  \end{myRule}
}
...
\newcommand{\ruleCallBeta}[1]{
  \begin{myRule}{#1}
    \AxiomC{$\subst (X\ \vecL{t}) = \pred\ \vecL{u}$}
...
\newcommand{\ruleImplM}[1]{
  \begin{myRule}{#1}
    % IMPL
    \AxiomC{\runCmd{\ConsHdNP{(\goalCmd{(H + \prog)}{B}{\alts})}\ConsTlNP{\vecL{g}}}{\alts[']}{\subst}{\alts['']}{\subst[']}}
    \RightLabelM{\ruleImpl}
    \UnaryInfC{\runCmd{\ConsHdNP{\goalCmd{\prog}{(\implCmd{H}{B})}{\alts}}\ConsTlNP{\vecL{g}}}{\alts[']}{\subst}{\alts['']}{\subst[']}}
  \end{myRule}
}
...
\newcommand{\rulePiM}[1]{
  \begin{myRule}{#1}
    % PI
    \AxiomC{\runCmd{\ConsHdNP{\goalCmd{(x + \prog)}{B}{\alts}}\ConsTlNP{\vecL{g}}}{\alts[']}{\subst}{\alts['']}{\subst[']}}
    \RightLabelM{\rulePi}
    \UnaryInfC{\runCmd{\ConsHdNP{\goalCmd{\prog}{(\piCmd{x}{B})}{\alts}}\ConsTlNP{\vecL{g}}}{\alts[']}{\subst}{\alts['']}{\subst[']}}
  \end{myRule}
}
...
We use the list processing combinator \fold %and \map 
...
A substitution $\subst$ of type $\Sigma$ is a pair $(s,m)$
where $s$ is a mapping from unification variables
\VAR to terms \TERM{} and $m$ is a constraint store.
We write $s\ t$ the application of the substitution $s$
to a term $t$, and $\subst\ t$ the  application of the first component
of \susbt to $t$.
...
$$
\run{} : \alt \times \vecL{\alt} \to (\vecL{\alt} \times \Sigma) + \bot
$$
...
\paragraph{Definition of $\bchain : TODO$}
...
\paragraph{Definition of $\mathcal{CHR} : TODO$}
...
the constraint just declared via \elpi{declare_constraint}.
...
Handling of names and hypotetical rules see~\cite{TASSI_2019}.
...
x, the expression must be put under a \elpi{pi y\\ }. Similarly
...
The key is the \elpi{copy} predicate~\cite{}, that reflects substitution
...
users know functional programming
