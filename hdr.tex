...
\subsection{Impreative terms, frames and trail}

In logic programming computation makes progress by assining unification
variables. The operation of assining and retrieving the assignment, if any,
has to be efficient. Even if not very frequently, at least not in out use cases,
assignments have to undone to implement backtracking.
Our experience with the first Elpi prototype suggested that purely functional
terms and an explicit assignment map was not a viable solution pushing us
...
engineering capabilities. As a result our terms are mutable and we store
a log of assignments to undo them.

The data type of terms is as follows:
...
the representation in terms of De Brijn levels in~\ref{sec:dbl}.\\
...
that we detail in~\ref{FFI}.\\
What is left are node for unification variables, and we postpone
the description of \ocaml{UVar} and \ocaml{Arg} to~\ref{sec:llambo}
...
by the FFI and a boolean flag to signal if it acts as a trigger for a constraint.
...
in the program corresponds to a function call that is usually implemented via
the allocation of an activation frame on the stack.\\
To be honest, in our runtime everything is allocated into the OCaml heap but
the stack frame becomes garbage (collectable by the garbage collector) as
soon as the sub goals for a rule are generated. As in the WAM there is the
invariant is that no heap term points to the stack.
Heap terms are longer lived, typically kept alive by an uvar present in the
initial query.

The \emph{depth} of an uvar is an integer representing the number of
\elpi{pi} generated fresh names existing when the uvar is created. We will write
$X^n$ the uvar $X$ with depth $n$. This is already an optimized representation
of unification variables since one could also use the list of explicit
...
When the \ocaml{contents} field of an \ocaml{uvar_body} assigned by
the unifier, the same \ocaml{uvar_body} is also logged in the trail.

\begin{ocamlcode}
type trail_item =
| UVarAssignement of uvar_body
...
\noindent
The trail also features two other kind of events one may need to undo, namely
the addition or removal of a constraint to the store, be we are not going to
provide any more detail on this. We rather focus on the higher-order part
of the language, the \ocaml{Const} node.

\subsection{Levels, not indexes}\label{sec:dbl}

In his famous paper~\cite{DEBRUIJN1994375} De Bruijn
...
languages based on the lambda calculus perform weak reduction, that in turn does
substitute closed term for the topmost variables. In DBI notation closed terms
are invariant by lifting, unlike the $x_0$ and $x_2$ that had to become a $x_1$.\\
...
By convention we name bound variables in De Bruijn Level representation as $c_i$
...
and this power is very handy in implementing substitution for the object
language. For example the following two lines implement weak head reduction
for the $\lambda$ calculus as encoded in~\ref{sec:hello} by simply recognizing
a redex and delegating to the $\beta$-reduction of $\lambda$Prolog the work
of pluggin $N$ inside $M$ whereever needed:
...
type checker in~\ref{sec:hello}, displayed below with an
...
is called $\beta_0$-reduction and it is suffient to execute $\lambda$Prolog
code in the $L_{\lambda}$ fragment, and in particular the code of
...
and to identify the \thefragment fragment, a restriction of the $L_{\lambda}$.
...
\begin{definition}[$L_{\lambda}$ fragment]
A term is in the $L_{\lambda}$
...
that, in ``some cases'' can be implemented in constant times when terms are
...
In~\cite{dunchev15lpar} we further restrict $L_{\lambda}$ as follows:
...
is applied to $c_j \ldots c_{j + k-1}$ for $k \leq 0$.
\end{definition}

% We allow $k = 0$ to accept variables that are not applied. 
Note that $X^j\ c_j \ldots c_{j + k-1}$ is equivalent to
a $Y^{j+k-1}$, and that a solution $t$ for $X$ can be always
represented as a term closed in the context $c_0\ldots c_{j+k-1} \vdash t$.

On the contrary the very same solution $t$ would require
binders if the variables was, say, $Z^0\ c_1\ c_0 \ldots c_{j+k-1}$ (note the first two arguments are flipped).
In this case one the solution would have been $\vdash \lambda b \lambda a\lambda c \ldots. t$.

Moreover, if an unification variable can be put in its compact form $X^n$,
then checking if a fresh name $c_m$ is in scope is a trivial as checking
$m < n$, no need to look for $c_m$ in the list of distinct arguments
the $L_{\lambda}$ fragment allows for.

Unification variables in \thefragment have an optimized representation in
Elpi, namely:
...
\noindent
where \ocaml{argsno} is the length of list of explicit arguments, e.g.
two in $X^n\ c_n\ c_{n+1}$.

It is also worth pointing out that, in DBL notation, the variables
bound by the context are invariant by shifting, hence ``moving around''
\ocaml{UVar} node is always efficient, unlike the more general case
of \ocaml{AppUVar} where the arguments may need shifting.
...
$\beta$-reduction). The Elpi test suite, that includes most of the
$\lambda$Prolog programs we could find online, was passing.
At some point I extended unification to the full $L_{\lambda}$ fragment.

\subsection{Indexing}

The runtime is equipped with three indexing algorithms, but before
presenting them it is worth pointing out another advantage of the DBL notation.
The again the rule to type check a lambda abstraction:
...
This simplifies indexing, since the hypothetical rule can be
indexed no $c_n$ with no further complication.

\subsubsection{Patricial tree (over bits)}

This index is the most standard one, like Prolog it is used to index one
argument at depth 1. Unlike standard Prolog the user can chose which argument
to index.
...
keyword), the index for each predicate is accompanied by two list of
rules, the ones to be returned in these two corner cases.
...
\elpi{global (indt "nat")} while the term \rocq{x + 0} is represented as\\
\elpi{app [global (const "plus"), x, global (indc "O")]}.

\subsubsection{Hash map}

If more than one argument needs indexing, or one argument needs
be indexed deeper, Elpi provides index based on the idea of
unification hashes\footnote{\url{http://blog.ndrix.com/2013/03/prolog-unification-hashes.html}}.

The hash value, a list of bits, is generated hierarchically and up to the
specified depth. Unification variables in a rule head are mapped to a
sequence of 1, while they are mapped to a sequence of 0 when they are part of
the query. Constants are mapped
to a hash value, a sequence of both 1 and 0. If the bit wise conjunction \ocaml{&}
of the hash of the query and the hash of the head of a rule is equal to the hash of
the query, then the rule is a match.

Intuitively:
\begin{itemize}
\item in a rule 1 means ``I provide this piece of info''
\item in the query 1 means ``I demand this piece of info''
\end{itemize}

A flexible argument in the query is made of 0s, hence it demands nothing, since \ocaml{0 & x = 0}
for any bit \ocaml{x} of the rule. Conversely a flexible argument in the rule is made of 1s,
hence it provides anything, since \ocaml{x & 1 = x} for any bit \ocaml{x} of the query.

For example consider the following code featuring a fast path:
\begin{elpicode}
kind nat type.
type o nat.
type s nat -> nat.
...
\noindent And imagine we index over 8 bits, and that the hash of \elpi{o} is
\ocaml{1001 1011} while the one of \elpi{s} is \ocaml{1011 0010}.

Hashes of each argument are obtained by combining the
hashes of the subterms, trimmed to fit word size.
...
implemented following Pfenning's class on automated theorem
...
linearization of a term (a tree). Paths are a sort of skip lists, i.e.
each node holds a pointer to its sibling (in the corresponding tree).
By concatenating paths one can easily apply the index to multiple arguments.

The main optimization is to flatten lists, i.e. a tree like\\
\elpi{app [global (const "plus"), x, global (indc "O")]}
is represented as if the \elpi{app} node had 3 siblings. Special care
is taken to handle open lists (when the tail is a unification variable).

For the majority of $\lambda$Prolog programs discrimination tree are slower
than the simpler, shallower, ones presented before but still find some 
applications. Some Elpi applications like Hierarchy-Builder, see~\ref{sec:hb},
make use of the program as a data base, accumulating to it tens of
thousands rules for the same predicate, with arguments not
very different from the term above.
All these rules can be discriminated
by a as-deep-as-needed index while they would result in an impractical number of collisions
with the other indexing techniques.

Another application is the determinacy checker added by Fissore~\cite{elpidet}
and in particular the test for rule with an overlapping head, that can be
performed easily and efficiently by indexing all heads with a discrimination
tree.

A possible application, yet to be explored, is to use discrimination trees
to implement a caching mechanism in the spirit of tabling.

\section{Compiler}

The compiler main job is to elaborate the syntactic sugar, i.e. namespaces
and spilling, and run a pretty standard type checker and, since Elpi
3.0, a determinacy checker.

The compiler performs very little optimizations. in particular is pre-processes
clauses by identifying the variables, so to know in advance the size of the
activation frame. This procedure is also needed at runtime when hypothetical
rules are loaded via implication.

One little optimization is to recognize lists and represent them with dedicated,
more compact, nodes in the syntax tree.

A component that is worth mentioning is the linker. Elpi supports
separate compilation, i.e. programs can be made of units that are
compiled at different point in time. In particular some Elpi programs
generate rules, see for example~\ref{sec:tb}, that are then compiled
and linked to the program next time it is executed.
A but unexpectedly this component became very important for the performance
of the compiler itself, since some relevant Elpi applications like Hierarchy-Builder,
see~\ref{sec:hb}, accumulate over time more than 40,000 rules. It was
important to craft a linker that is as incremental as possible, i.e. its
computational complexity does (mainly) the unit extending the base
program, and not on  the base program itself.
...
if a reference \ocaml{x} was originally given the value 42, or
...
The \ocaml{B.ioargC} adaptor lifts a contextual conversion to the option
type: if actual data is provided, then it is converted.
The \ocaml{B.diagnostic} datum is either the Elpi value
\elpi{ok} or \elpi{error "message"}, hence a call \elpi{rocq.typecheck T Ty ok}
asserts that \elpi{T} is well typed of type \elpi{Ty}.

The only missing piece is \ocaml{sigma}, the environment assigning
type judgements to holes, i.e. the call to the \ocaml{get_sigma} and
\ocaml{set_sigma} API.
...
In Elpi we represent this data, to which we refer as \ocaml{sigma},
as a set of constraints \elpi{evar}.
Such constraint links a ``raw'' Elpi unification variable \elpi{RawEv},
a type and the ``elaborated'' Elpi unification variable \elpi{Ev} (the meaning
is clarified below).
The constraint should live in a context of hypothetical rules
\elpi{Ctx} assigning types to bound variables. In other words
...
In order to declare a new \elpi{evar} constraint in any moment, even if
...
\noindent
Note how \elpi{declare-evar} uses a CHR rule to craft
a new goal with a precise context \elpi{Ctx}, no matter the context
...
The only missing bit the \ocaml{get_sigma} and \ocaml{set_sigma} APIs.
Their role is synchronize updates to \ocaml{sigma} in both directions.
The former, \ocaml{get_sigma}, is the simplest since a Rocq
datum of type \ocaml{evar_map} is stored into the extensible state
of Elpi.
...
one for ``raw'' holes and one for ``elaborated'': a Rocq \ocaml{Evar.t}
is linked to one of each.
In order to propagate and update to \ocaml{sigma} performed by Rocq we 
fold over the set of linked evars to check if the were assigned. If so
we remove the corresponding \elpi{evar} constraint and we also generate
