...
\newminted[coqcode]{theorem.py:XCoqLexer}{fontsize=\small,autogobble,escapeinside=~~,mathescape=true,frame=leftline,framerule=0pt,framesep=1em}
...
characteristic of logic programming is of being rule based.
...
one, in a larger context. Even the definition of a self contained function fails
...
The main data type Coq-Elpi programs work with is the one of
...
contextual conversion can hence access a Coq context when operating its
...
are read back in the \ocamlinline{proof_context.env} Coq context,
...
Each Coq hole as an entry in \ocamlinline{sigma} that looks like a
...
under which \elpiinline{declare-evar} is generated. Ineed a Coq API
that is called deep inside a term and that encounters a new hole has
will add to Coq's sigma an entry for it, but also generate a
\elpiinline{declare-evar}, and hence an \elpiinline{evar} constraint
in Elpi.

The code above also relates a Coq hole \elpiinline{Ev} and an
...
In the code above \elpiinline{coq.elaborate-skeleton} is an API
very similar to \elpiinline{coq.typecheck} but that does not modify
...
that Coq runs on any input term written by the user, while \elpiinline{coq.typecheck}
corresponds to code internally used by Coq tactics to build well typed terms.

The extensible state~\ref{sec:API} offered by Elpi carries a Coq-Elpi specific
bidirectional map linked Coq unification variables and Elpi holes.
...
TODO sigma in Elpi, sigma in Coq, going from both.
...
That vernacular language of Coq lets one organize the formalized knowledge,
...
be executed in Coq proof mode.

\subsection{Commands}

A command is declared with \coqinline{Elpi Command} followed by a name.
Initially a command only contains the glue code for Coq, like the
HOAS data type of terms and the builtin predicates. Extra code
can be added to a command via the \coqinline{Elpi Accumulate}.
...
A command \texttt{hello} is executed with \coqinline{Elpi hello} followed
...
The simple program above calls the \elpiinline{coq.say} builtin whenever the
...
it checks that no Coq proof variable is already using the same name, and
...
and also to eltac~\cite{app:eltac}, a collection of Coq tactics rewritten in Elpi for
didactic purpose.
...
rules. The \coqinline{Elpi Db} commands declares a data base.
...
We do not expect the users of Coq commands written in Elpi to know the
syntax of Elpi, not the commands to manipulate its programs, and even more
to actually know the Coq commands they run are implemented in Elpi.

For this reason Coq-Elpi provides APIs to extend data bases from Elpi.
Here we craft a new command \coqinline{set_age} that does use this API.
...
commands or tactics are used in a Coq library.
...
\chapter{Coq tools written in Elpi}

In this chapter we survey the some tools developed on top of Coq-Elpi.
...
At the end of each section we discuss which features of Elpi and Coq-Elpi
...
\subsection{The role of Coq-Elpi in derive}
...
the automatic handling of names and holes was key. Only a few Coq APIs were
...
\subsection{The role of Coq-Elpi in Hierarchy Builder}
...
by calls to Coq API, typically to typecheck and define the terms that are
...
Algebra tactics is a port (and improvement) of the \coqinline{ring}, \coqinline{field} and
\coqinline{lra} tactics to the Mathematical Components library~\cite{sakaguchi:LIPIcs.ITP.2022.29}.
...
inherited from a substructure. Since the type theory of Coq does not feature
...
\subsubsection{The role of Coq-Elpi in Algebra Tactics}

The code crytically uses the possibility to call
the \elpiinline{coq.unify-eq} deep inside the goal to identify
terms up to (Coq) unification.
...
The \coqinline{ring} tactic takes about five seconds to solve it.
...
are computable, but are not immediately executable by Coq for one of
...
the Coq type checker to potentially take an unreasonable amount of time.
...
theory of Coq, that does not internalize the principle of univalence, is
sufficient to represent the result of the parametricy translation
for non bare-bone relations.

\subsubsection{The role of Coq-Elpi in Trocq}
...
all requirements are know he computes the minimum set of Coq axioms
...
program logic for C++. It is a set of Coq libraries, partially based on Iris,
...
The employ Coq-Elpi to synthesize code automatically extending the
...
We focus on another use they make of Coq-Elpi in the form of its
NES application.

\subsubsection{N.E.S. -- Namespace Emulation System}

It is folklore that Coq modules can, somehow, emulate name spaces.
Coq modules are inspired by OCaml modules and are a closed entity: when
...
in Coq, of a so called ``nametab'': an environment of non-ambiguous names
that can be extended by importing in it the names from a module, even the
names of modules themselves. It allows to create the illusion that \coqinline{A.x}
and \coqinline{A.y} are items from the same module \coqinline{A} even if
the two \coqinline{A}s have different (long) names, eg \coqinline{X1.A.x}
and \coqinline{X2.A.y}.
...
The basic features of NES can be seen here in the Coq code below:
...
It worth pointing out how the definition of \coqinline{more_stuff}
...
\subsubsection{The role of Coq-Elpi in BRiKs}
...
patches and early feedback to Elpi and Coq-Elpi.  
...
We take the liberty to challange the reader to write the \coqinline{to_bool}
...
What we love the most about our job is to design and build tools~\cite{matita,ssr,coq}.
...
Coq-Elpi totals 13K: 5K for the HOAS of Coq terms, 5K for the bindings of about
300 Coq APIs and 3K for the vernacular language to declare programs. The complicated
part is the HOAS, but it is also what makes the language shine and enables
each Coq API to require, on average, only 16 lines to be exposed to Elpi
...
Coq-Elpi was developed in the open from the the very beginning, making the
Coq development team aware of the effor. Their committment to
provide patches to Coq-Elpi eavery time a Coq API did evolve made its
development over more than seven years sustainable. The contribution 
from the Coq community was also very important, in particular from
power users such as BlueRock systems that contributed a substantial amount
of code.

\subsection{Coq tools written in Elpi}
