...
that is has the expected type. When the term \rocq{a} of type \rocq{A},
the type parameter of the container, then it comes with a proof of \rocq{isA a},
the predicate associated with \rocq{A}.
...
Parametricity is a meta theorem in Rocq, a theorem about Rocq itself.
Since it cannot be represented by a Gallina term one
needs a meta language to effectively implement it.
...
I based the unary version on his code. Each translation is about 250 lines
of Elpi code.
...
it allowed to quickly and concisely write derivations. Some of the derivations
like the one for deep inductive principles, were not know before, and the ease
of experimentation was intrumental to discover them.
...
Performance was not crytical: all bottlenecks turned out to be inefficiencies
...
type classes instances for the \rocq{EqDecision}, \rocq{Countable},
\rocq{Finite} and \rocq{Inhabited} std++ classes, as well as a serialization
one called \rocq{ToBit}. Finally
